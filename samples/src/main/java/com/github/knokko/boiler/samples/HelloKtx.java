...
//		}
	...
	protected void setup(BoilerInstance boiler, MemoryStack stack) {
		...
		for (int index = 0; index < 4; index++) { // TODO Use compressed textures for the last 3 indices
			var simpleImageWrite = writeImages.get(index);
			simpleImageWrite.imageView(this.simpleImage.vkImageView());
			simpleImageWrite.imageLayout(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
		}
		...
		this.pipelineLayout = boiler.pipelines.createLayout(null, "CompressedLayout", descriptorSetLayout.vkDescriptorSetLayout);
        ...
        try {
			var bufferedImage = ImageIO.read(Objects.requireNonNull(HelloKtx.class.getClassLoader().getResourceAsStream(
					"com/github/knokko/boiler/samples/images/test1.png"
			)));
		...
		var stagingCommandPool = boiler.commands.createPool(0, boiler.queueFamilies().graphics().index(), "StagingCommandPool");
